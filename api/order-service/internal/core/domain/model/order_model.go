...
type Order struct {
	...
	BuyerID          int64             `gorm:"type:bigint;not null;index"`
	...
	BuyerEmail       string            `gorm:"type:varchar(255);not null"`
	BuyerAddress     string            `gorm:"type:text"`
	BuyerPhone       string            `gorm:"type:varchar(17)"`
	BuyerLat         string            `gorm:"type:varchar(50)"`
	BuyerLng         string            `gorm:"type:varchar(50)"`
	CreatedAt        time.Time         `gorm:"type:timestamp;default:current_timestamp"`
	UpdatedAt        time.Time         `gorm:"type:timestamp"`
	DeletedAt        *gorm.DeletedAt   `gorm:"index"`
	OrderItems       []OrderItem       `gorm:"foreignKey:OrderID;constraint:OnDelete:CASCADE"`
	ProductSnapshots []ProductSnapshot `gorm:"foreignKey:OrderID;constraint:OnDelete:CASCADE"`
}
