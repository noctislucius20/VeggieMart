...
func (c *categoryCache) DeleteCategoryCache(ctx context.Context, id int64) error {
	...
	if err := helper.RedisBulkGet(ctx, c.redisClient, keys, &category); err != nil {
		c.logger.Errorf("[CategoryCache-1] DeleteCategoryCache: %v", err)
		return err
	}
	...
	if err := c.redisClient.Del(ctx, delKeys...).Err(); err != nil {
		c.logger.Errorf("[CategoryCache-2] DeleteCategoryCache: %v", err)
		return err
	}
...
func (c *categoryCache) GetCategoryById(ctx context.Context, id int64) (*entity.CategoryEntity, error) {
	...
	if err == nil {
		...
		if val == "null" {
			...
			c.logger.Errorf("[CategoryCache-1] GetCategoryById: %v", err)
			return nil, err
		}
	...
	if err != nil {
		...
		if err.Error() == utils.DATA_NOT_FOUND {
			if err := c.redisClient.Set(ctx, key, "null", 1*time.Minute).Err(); err != nil {
				c.logger.Errorf("[CategoryCache-2] GetCategoryById: %v", err)
			}
		}

		c.logger.Errorf("[CategoryCache-3] GetCategoryById: %v", err)
		return nil, err
	}
	...
	if err := c.redisClient.Set(ctx, key, jsonData, 1*time.Hour).Err(); err != nil {
		c.logger.Errorf("[CategoryCache-4] GetCategoryById: %v", err)
	}
...
func (c *categoryCache) GetCategoryBySlug(ctx context.Context, slug string) (*entity.CategoryEntity, error) {
	...
	if valKey, err := c.redisClient.Get(ctx, key).Result(); err == nil {
		...
		if valKey == "null" {
			err := errors.New(utils.DATA_NOT_FOUND)
			c.logger.Errorf("[CategoryCache-1] GetCategoryBySlug: %v", err)
	...
	if err != nil {
		...
		if err.Error() == utils.DATA_NOT_FOUND {
			if err := c.redisClient.Set(ctx, key, "null", 1*time.Minute).Err(); err != nil {
				c.logger.Errorf("[CategoryCache-2] GetCategoryBySlug: %v", err)
			}
		}

		c.logger.Errorf("[CategoryCache-3] GetCategoryBySlug: %v", err)
		return nil, err
	}
	...
	if err := pipe.Set(ctx, key, category.ID, ttl).Err(); err != nil {
		c.logger.Errorf("[CategoryCache-4] GetCategoryBySlug: %v", err)
	}
	if err := pipe.Set(ctx, fmt.Sprintf("category:id:%d:slug", category.ID), jsonData, ttl).Err(); err != nil {
		c.logger.Errorf("[CategoryCache-5] GetCategoryBySlug: %v", err)
	}
	if _, err = pipe.Exec(ctx); err != nil {
		c.logger.Errorf("[CategoryCache-6] GetCategoryBySlug: %v", err)
	}
